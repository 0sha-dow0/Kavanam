...
async function afterPackageBuilt (packagePath) {
  ...
  /* create zip files */
  var output = fs.createWriteStream('dist/app/' + 'OnTask-v' + version + '-windows' + archSuffix + '.zip')
  var archive = archiver('zip', {
    zlib: { level: 9 }
  })
  archive.directory(packagePath, 'OnTask-v' + version)
  archive.pipe(output)
  ...
  const options = {
    src: packagePath,
    dest: 'dist/app/ontask-installer' + archSuffix,
  ...
  await installer(options)
    .then(function () {
      fs.renameSync('./dist/app/ontask-installer' + archSuffix + '/ontask-' + version + '-setup.exe', './dist/app/ontask-' + version + archSuffix + '-setup.exe')
    })
    .catch(err => {
